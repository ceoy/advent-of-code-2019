...
def part1(boost_program)
  ...
  result = intcode.do_instruction(colors[:black])
...
def part2(boost_program)
  ...
  # intcode will input once, then output twice then repeat
  result = intcode.do_instruction(colors[:white])

  # init pane
  pane={}
  pane[0] = {0=>Field.new(false, colors[:white])}
  ...
  (min_y..max_y).each { |h|
    ...
    (min_x..max_x).each { |w|
      if pane[h][w] == nil
        pane[h][w] = Field.new(false, colors[:black])
      end
    }
  }
  ...
  (min_y..max_y).each { |y|
    (min_x..max_x).each { |x|
      if pane[y][x].color == colors[:black]
        print "."
      else
        print "#"
      end
